...
# ИСПРАВЛЕННАЯ ФУНКЦИЯ: Теперь она только обрабатывает одну группу (Тикер)
def generate_target(group):
    """
    Создает целевую переменную: sign(return(D+1, D+2)) для одной группы (Тикера).
    """
    # Шаг 1: Рассчитываем цены Close(D+1) и Close(D+2) на строке D
    group['close_d+1'] = group['close'].shift(-1)
    group['close_d+2'] = group['close'].shift(-2)

    # Шаг 2: Рассчитываем возврат return(D+1, D+2)
    ...
    # Примечание: Target = 0 (нет изменений) может вызвать проблемы с AUC, 
    # поэтому обычно их исключают или используют Multiclass классификацию.
    # Пока оставим 1/-1/0.
